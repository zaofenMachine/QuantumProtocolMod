...
> 2026-09-13 当前续接入口：[第三十阶段](phase-30-generated-player-card.md)。继续“小关重开 + 玩家精确恢复”。v0.31.0 的 FIELD schema 6 支持春正常生成的一张额外未升级苹果，撤销临时启动对生命上限和倒计时的影响，永久牌组仍保持原样；恢复后攻击退场与跨进程再保存通过。刚生成的春仍有 INIT 完成动作显示历史差异，报告明确未恢复该项。下一步补齐生成卡存在但场地已清空的牌区／墓地流程，再推进真实场外动态状态。重编程与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
