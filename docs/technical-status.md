...
> 2026-09-13 当前续接入口：[第二十七阶段](phase-27-native-player-counters.md)。继续“小关重开 + 玩家精确恢复”。v0.28.0 增加普通计数器与逐标签特殊计数器的原生只读观测和独立比较；真实样本证明相同总量可有不同标签分配，特殊条目会保留零值且允许负值。FIELD schema 4 的攻击、生命修正和高于上限的当前生命恢复已通过正式回归。下一项接入 FIELD 计数器恢复；非零独立生命调整、LEVEL 和额外生成卡继续推进。重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
