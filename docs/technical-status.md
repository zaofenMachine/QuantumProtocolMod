...
> 2026-09-13 当前续接入口：[第二十一阶段](phase-21-restore-object-filtering.md)。继续“小关重开 + 玩家精确恢复”，暂时搁置重编程重开与敌人精确恢复。v0.22.0 在原生类型筛选后保留原对象校验，降低恢复扫描开销；满手牌场地、混合场地／墓地、全墓地与旧格式恢复通过，场地组合顺序干扰已实际触发回退。下一项读取原生攻击力与属性修正。现有比较只证明已采集字段，不能外推全部动态状态。下面的旧调试清单保留为历史材料。
