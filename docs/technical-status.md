...
- 正式跨进程恢复倾向 UE4SS C++ Mod。只读导出器 v0.1–v0.3 已使用 RE-UE4SS v3.0.1、UEPseudo、MSVC 14.38 和固定 Rust nightly 完成构建、部署与真实战斗验证；三版报告分别包含 23、33 和 99 个相关对象，均未导致游戏崩溃。
...
1. 在实际非零场景验证通用/特殊计数器，并收集带持续效果、多个敌人与特殊 Spawner 的样本。
2. 区分效果显示状态与权威效果执行状态，定位安全的读取与写回 API。
3. 把 UE `ExportTextItem` 输出转换为版本化、可校验的结构化检查点数据。
4. 验证从菜单进入指定流程/关卡的安全路径与加载时机。
5. 设计可丢弃测试局中的单敌人创建/写回实验，再回到 [architecture-options.md](architecture-options.md) 做最终路线决策。
