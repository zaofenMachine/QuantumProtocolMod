...
> 2026-09-13 当前续接入口：[第二十八阶段](phase-28-player-field-counters.md)。继续“小关重开 + 玩家精确恢复”。v0.29.0 的 FIELD schema 5 已保存、恢复并持续校验普通计数器及完整特殊标签表，包含零值和负总量；攻击、生命与计数器组合以及两张同名牌的差异状态通过正式回归。相同总量、不同标签的外部变化会触发唯一一次语义回退。下一项确认 LEVEL 与独立生命上限调整，继续补齐场外牌动态状态和额外生成卡。重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
