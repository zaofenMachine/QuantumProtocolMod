...
> 2026-09-13 当前续接入口：[第二十二阶段](phase-22-native-player-statistics.md)。继续“小关重开 + 玩家精确恢复”。v0.23.0 已补原生基础／当前攻击力和属性修正只读导出，实际观察到春生成苹果的 springBuff +1 及执行后清除。独立比较对新增字段单独报告可用性，缺失为未知。普通开局和满手牌场地的新增统计比较通过。下一项验证原生属性修正写入、清除及回滚，再接入精确恢复；带增益的额外生成卡目前只用于观测。重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
