...
> 2026-09-13 更新：继续“小关重开 + 玩家精确恢复”，重编程重开和敌人精确恢复暂时搁置。v0.18.0 确认旧排序 Getter 无法代表真实抽牌、手牌及墓地顺序，改为读取原生数组并写入玩家牌区 schema 2，恢复后按同一来源终验；旧格式保留兼容并标为 legacy-sorted-only。证据与边界见 [第十七阶段](phase-17-native-player-zone-order.md)。跨手/场/墓的同身份卡和更多玩家动态状态仍待单独闭合。
