...
> 2026-09-13 当前续接入口：[第二十五阶段](phase-25-native-player-health.md)。继续“小关重开 + 玩家精确恢复”。v0.26.0 增加玩家有效生命上限的原生只读观测和独立比较，缺失字段为未知。实机 HEALTH +3 衰减后，上限从 5 回到 2，当前卡牌生命仍保持 5，证明现有“当前生命不超过基础值”的限制会漏掉合法状态。下一项将受限 FIELD HEALTH 修正纳入补充文件，在修正表恢复后写入最终当前生命。非零原生上限调整、LEVEL、计数器和额外生成卡仍待后续处理。重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
