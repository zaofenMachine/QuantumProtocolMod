...
> 2026-09-13 当前续接入口：[第二十六阶段](phase-26-player-field-health.md)。继续“小关重开 + 玩家精确恢复”。v0.27.0 的 FIELD schema 4 已接入原生 HEALTH 修正，支持与 ATTACK 同时存在，先重建修正表再写入最终当前生命，保留生命高于基础值／有效上限的合法状态。旧 schema 1／2／3 保持兼容，并明确缺少生命上限覆盖。实机已验证正负修正、正常回合衰减，以及只改生命状态时单次 Route C 回退。正式构建验收与样本见对应证据目录。非零原生上限调整、LEVEL、计数器和额外生成卡继续推进；重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
...
当前结论更新：2026-09-13。以下为截至 2026-09-09 的阶段记录。
