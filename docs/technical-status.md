...
> 2026-09-20 当前续接入口：[第三十六阶段](phase-36-player-hand-base-health.md)。既定方向为“小关重开 + 玩家精确恢复”；用户最新要求当前任务完成后停下汇报，本轮只做收尾，不再启动新功能。下方旧调试内容仅保留历史证据。

## 2026-09-20 当前状态

当前 v0.37.0 已完成 HAND3 基础生命增长、HEALTH 修正与 generic 计数的联合恢复切片。HAND3 沿用已有字段，按完整 CardInfo 与原生 HAND 下标绑定每张卡，只接受相对定义的非负基础生命增长、正数且有界的生命、非负 HEALTH 修正、当前生命等于上限及无独立生命调整；攻击、等级和回合仍须符合场外门禁。DECK/TRASH/FIELD 的非默认基础生命不随本轮放开，HAND special 计数仍不支持。旧 HAND 1/2 必须保持定义基础生命，HAND3 要求选中的 Z4/T5/F8 布局具有保存时技能成员证明。

正常操作已确认焰火经上升涡流回手后基础生命 1→2，奥克塔娃再给予 HEALTH +1 后为 3/3；恢复与再次正常出牌的重置行为均已验证。同定义的两张焰火分别保存基础生命 1 与 2，以原生位置区分，不能按名称聚合。原生基础生命动作会清掉 HEALTH 修正，因此恢复先完成布局，再依次写基础生命、HEALTH 修正和 generic 计数。

恢复搬牌原先会触发焰火自身的成长。当前仅在恢复专用 MOVE 期间临时抑制被搬动目标的原生技能数量，确认动作完成、延迟队列为空且到达目标牌区后，整批还原并核验成员；异常清理只负责还原。其间产生的技能显示缺项通过游戏原生同步函数补齐。Flashbang 留下非前缀显示子集的实机样本已出现 `[1] → [1,0] → [0,1]`：唯一合法子集补齐后仅重排 UI 指针数组，保留已有 actor/widget/history，最后仍要求完整原生顺序。日志直接记录的是序号与保留数量，实例身份保持由产品内部门禁验证；没有放宽未知显示或历史差异。

第三十五阶段的全玩家技能成员门禁继续生效：保存与恢复均核对 DECK/HAND/TRASH/FIELD 的原生工厂键和选中定义；Z4/T5/F8 标明保存时证明，旧布局报告 `legacy-unknown`。真实病毒删技能、成长焰火进入 DECK/TRASH 的样本分别验证了成员与数值拒绝，不能计作联合恢复成功。七类现行补充及其备份仍在发布新主文件前失效。

当前生产 DLL 为 `C9D10C5FDD391D1E9652A4961E9B42E45C204FF2B2F8F375A0EA70E6F5497C95`，构建源码 SHA-256 为 `B5C5936A3A6C824637D105E22B67782248DF59285430E16A0B22E1D9476D0529`。开发夹具关闭，4/4 CTest 通过。截至本次入口更新，验收分开记录：

- 14 组生产恢复完成全部 25 项玩家状态对照，包含原始用户检查点最终恢复和修正默认样本比较基线后的结果；原始比较错误与真实失败仍保留。
- 2 组旧清单仅按其历史可用范围通过，随后重新保存并建立新的原生观测基线完成对照；不把旧清单缺失的观测补记为已证明。
...
## 早期阶段待验证项（历史记录，不是当前计划）

以下列表保留当时的调查顺序；当前推进方向与已完成范围以文首和第三十六阶段为准。
