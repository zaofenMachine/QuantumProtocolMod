...
> 2026-09-13 当前续接入口：[第二十三阶段](phase-23-player-field-attack.md)。继续“小关重开 + 玩家精确恢复”。v0.24.0 将 FIELD 攻击修正接入 schema 3 保存、原生队列恢复与最终持续校验；旧格式仍可读并明确标注攻击记录缺失。满手牌、同名场地不同加成、实际单次攻击消耗和攻击状态干扰后的单次回退均已验证。下一项验证独立衰减、负修正与 limit 行为，再扩展玩家动态状态。额外生成卡仍超出当前恢复范围，重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
