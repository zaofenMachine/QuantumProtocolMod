...
> 2026-09-13 当前续接入口：[第二十九阶段](phase-29-native-player-level.md)。继续“小关重开 + 玩家精确恢复”。v0.30.0 新增等级原生观测和独立比较；实机确认 LEVEL 条目可衰减，但原生等级 getter 不会把其数量加到卡牌等级。FIELD schema 5 的位置、攻击、生命、计数器恢复继续通过正式回归；LEVEL 条目和非零独立生命调整仍明确排除。下一项优先处理“春”正常生成额外苹果后的卡牌集合缺口，再补齐场外动态状态。重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
