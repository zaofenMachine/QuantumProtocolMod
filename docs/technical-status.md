...
> 2026-09-13 当前续接入口：[第二十四阶段](phase-24-player-attack-lifecycle.md)。继续“小关重开 + 玩家精确恢复”。v0.25.0 修复正负攻击修正的重放顺序，避免原生逐步归零破坏负修正。正负共存、攻击归零、limit 上限、独立 DECAY 逐回合衰减以及恢复后的到期均已验证。下一项继续补生命上限等玩家动态属性的权威观测与恢复。额外生成卡仍超出当前恢复范围，重编程重开与敌人精确恢复继续搁置。下面的旧调试材料保留为历史记录。
