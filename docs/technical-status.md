...
- 当前实现：C++ v0.9.13 路线 C 核心垂直切片已闭环。schema 2 保存活动牌组、缓存、新掉落物、关卡/角色、波次和生命；恢复使用新战斗上下文与分阶段 BeginPlay 语义重放。同一最终构建连续三轮报告 `passed`，特殊资源、三张新掉落物 UI、牌组、缓存和生命均通过实机验收。
...
1. 建立精确状态的只读基线与差异报告：同一保存点在恢复前后分别导出玩家各牌区、敌方实例、精确槽位、生命、回合计数、计数器和效果，按稳定身份字段匹配并分类差异。
2. 根据差异规模选择一个最小写回切片。优先选择已有权威字段、已有 UI 交叉验证且能自动回滚的生命/回合或单一区域顺序，不直接尝试一次重建全部战场。
3. 精确层保持为路线 C 之上的可选阶段；任何精确写回失败都应留下已经通过的语义恢复结果，不得破坏关卡重入、牌组、新掉落物和生命的稳定路径。
