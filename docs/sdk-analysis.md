...
C++ 导出器已部署并完成多轮真实战斗验证，生命与回合计数也已通过受控跨帧写回。下一批证据集中在仍未闭合的复合状态：

1. 在真实非零样本中关联通用/特殊计数器的显示对象、卡牌实例与权威执行状态。
2. 对持续效果、临时属性修改、自动化与保护状态比较显示层和原生状态。
3. 盘点特殊 `SpawnController`、Boss 监视器和关卡辅助 Actor 的最小持久化集合。
4. 把已验证字段转换为版本化、可校验、原子替换的独立检查点格式。
5. 验证从菜单进入指定流程/关卡的加载时机，再设计暂停动作队列、创建对象、依赖顺序写回和失败回滚。
