...
当前 v0.34.0 沿用路线 C“普通地牢小关语义重开”，叠加受限的玩家精确恢复：原生牌序、手牌数量、场格、生命与攻击修正、计数器、抽牌进度，以及春生成的一张额外苹果。恢复在游戏线程执行，精确层失败后重新加载一次纯 Route C。
...
v0.34.0 的 HAND schema 2 按原生下标保存生命与 generic 计数，沿用现有布局依赖；旧 HAND schema 1 保持兼容。恢复采用原生动作并逐帧核验，不通过时仅回退一次普通 Route C。正式 DLL 关闭开发夹具，4/4 CTest 与实机验收已完成。部分技能的 COMPLETE→NONE 历史差异仍存在，不宣称全量快照等价。下一步先补原生效果观测，查清同进程缺口，再验证 sorc 回手的动态基础生命。详见：
